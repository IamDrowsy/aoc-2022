...
export default class Solution extends AbstractSolution<Pair[], Packet[]> {
  public parseInput1(input: string[]): Pair[] {
    return chunk(input, 3).map((lines) => [
      JSON.parse(ensure(lines[0])),
      JSON.parse(ensure(lines[1])),
    ]);
  }

  public parseInput2(input: string[]): Packet[] {
    return this.parseInput1(input).reduce(
      (all, [p1, p2]) => [...all, p1, p2],
      dividerPackets
    );
  }
  ...
  public solveFirst(input: Pair[]): string {
    return sum(
      input.map((p, i) => (checkOrder(p) === "Right" ? i + 1 : 0))
    ).toString();
  }
  public solveSecond(input: Packet[]): string {
    return product(
      input
        .sort((p1, p2) => (checkOrder([p1, p2]) === "Right" ? -1 : 1))
        .map((p, i) => (dividerPackets.includes(p) ? i + 1 : 1))
    ).toString();
  }
}
